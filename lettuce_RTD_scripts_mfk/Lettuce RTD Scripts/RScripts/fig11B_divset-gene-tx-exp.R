rm(list = ls())
# Gene and Transcript Expression (Lsat_DivSet_Sscl)
# load libraries ----------------------------------------------------------
require(tidyverse)
require(readxl)
...
## read RNAseq info -------------------------------------------------------
wdir <- "./Chapters/ch4/"
df <- readxl::read_xlsx(path = paste0(wdir, "suppl/suppl_3.xlsx"),
                        sheet = "Table S3",
                        skip = 4)
...
plt <- df %>%
  ...
  geom_text(aes(label = count, colour = exp), vjust = 1.6) +
...
# save plot ---------------------------------------------------------------
png(
  file = paste0(wdir, "figures/divset_gene_tx_exp.png"),
  width = 6,
  height = 4.5,
  units = "in",
  res = 300
...
svg(
  file = paste0(wdir, "figures/divset_gene_tx_exp.svg"),
  width = 6,
  height = 4.5,
)
plt
