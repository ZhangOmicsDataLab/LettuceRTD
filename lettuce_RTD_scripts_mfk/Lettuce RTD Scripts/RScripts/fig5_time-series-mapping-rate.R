# Mapping Efficiency (Lsat_Bcin_timeseries)
rm(list = ls())
# load libraries ----------------------------------------------------------
require(tidyverse)
require(readxl)
...
multiqc_salmon <- multiqc_salmon |>
  ...
  mutate(
    ref_tx_idx = ifelse(
      ref_tx_idx == "Quant_GCA_002870075.4_Lsat_Salinas_v11_GCA_000143535.4_ASM14353v4_merged_salmon_index",
      "GenBank",
      ifelse(
        ref_tx_idx == "Quant_GCF_002870075.4_Lsat_Salinas_v11_GCA_000143535.4_ASM14353v4_merged_salmon_index",
        "RefSeq",
        "LsRTDv1"
      )
    )
...
plot(GenBank, jitter(LsRTDv1),    # jitter offsets points so you can see them all
     pch = 16,                 # shape of points
     cex = 1.0,                # size of points
     xlim=c(50, 110),          # limits of x-axis
     ylim=c(50, 110),          # limits of y-axis
     xlab="GenBank",            # label for x-axis
     ylab="LsRTDv1")             # label for y-axis

text(GenBank, LsRTDv1, labels=Samples,  # Label location and text
     
     pos=3, cex=1.0)               # Label text position and size

abline(0, 1, col="blue", lwd=2)     # line with intercept of 0 and slope of 1

barplot(LsRTDv1vsGenBank.Diff,                    # variable to plot
        col="dark gray",                          # color of bars
        xlab="Samples",                           # x-axis label
        ylab="Difference (LsRTDv1 - GenBank)",    # y-axis label
        names.arg=Samples)                        # labels for bars

plot(RefSeq, jitter(LsRTDv1),  # jitter offsets points so you can see them all
     pch = 16,                 # shape of points
     cex = 1.0,                # size of points
     xlim=c(50, 110),          # limits of x-axis
     ylim=c(50, 110),          # limits of y-axis
     xlab="RefSeq",            # label for x-axis
     ylab="LsRTDv1")           # label for y-axis

text(RefSeq, LsRTDv1, labels=Samples,  # Label location and text
     
     pos=3, cex=1.0)               # Label text position and size

abline(0, 1, col="blue", lwd=2)     # line with intercept of 0 and slope of 1

barplot(LsRTDv1vsRefSeq.Diff,                    # variable to plot
        col="dark gray",                          # color of bars
        xlab="Samples",                           # x-axis label
        ylab="Difference (LsRTDv1 - GenBank)",    # y-axis label
        names.arg=Samples)                        # labels for bars
...
stat.test <- multiqc_salmon %>%
  pairwise_t_test(
    percent_mapped ~ ref_tx_idx, paired = TRUE, 
...
# Create the plot
bxp <- ggboxplot(
  multiqc_salmon, x = "ref_tx_idx", y = "percent_mapped",
  color = "ref_tx_idx", palette = "jco"
)
...
plt <- multiqc_salmon |>
  ...
  gghalves::geom_half_boxplot(outlier.color = NA,
                              position = position_nudge(x = -.01)) +
  showSignificance( c(2,3), 90, 1, "**** \n Cohen's d = 1.7",
                    textParams    = list(size = 4)) + 
  showSignificance( c(1,3), 75, 1, "**** \n Cohen's d = 7.0", textParams    = list(size = 4)) + 
  scale_y_continuous(
    limits = c(print(
...
# save plot ---------------------------------------------------------------
png(
  file = paste0(wdir, "figures/time-series_mapping_rate.png"),
  width = 6,
  height = 4.5,
  units = "in",
  res = 300
...
svg(
  file = paste0(wdir, "figures/time-series_mapping_rate.svg"),
  width = 6,
  height = 4.5
)
