plot.timeseries <- function(gene.id) {
  ...
  AHRD_func_annot <- read.csv("U:/Lettuce_RTD_analysis/P08E96_references/LettuceRTDv1/func_annot/AHRD/AHRD_func_annot.csv")
  AHRD_func_annot <- AHRD_func_annot |>
    distinct(gene.id, .keep_all = T) |> 
    filter(gene.id == {{ gene.id }})
  if (is.na(AHRD_func_annot$ahrd.desc)) {
    title = paste0(AHRD_func_annot$gene.id)
    subtitle = NULL
  } else {
    title = paste0(AHRD_func_annot$gene.id)
    subtitle = paste0(AHRD_func_annot$ahrd.desc
                      # "\n",
                      # "BLAST Hit: ",
                      # AHRD_func_annot$source,
                      # "|",
                      # AHRD_func_annot$organism,
                      # "|",
                      # AHRD_func_annot$primary.acc,
                      # " [",
                      # AHRD_func_annot$ahrd.quality.code,
                      # "]"
                      )
  }
